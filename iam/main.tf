...
resource "oci_identity_user_group_membership" "membership" {
    ...
    user_id = lookup(values(data.oci_identity_users.users[*].name), each.value["user_name"]).id
    group_id  = lookup(values(data.oci_identity_groups.groups[*].name), each.value["group_name"]).id
}
